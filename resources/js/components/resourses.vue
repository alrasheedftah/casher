          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">جدول المكونات</h3>
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                  ...
                  <tbody>
                  <tr v-for="resourse in resourses.data " :key="resourse.id">
                  <td>{{resourse.id}}</td>
                  <td>{{resourse.name}}</td>
                  <td>{{resourse.description}}</td>
                  <td>{{resourse.packaging_type}}</td>
                  <td>{{resourse.quantity}}</td>
                  <td><span class="tag tag-success">{{resourse.icon}}</span></td>
                      <td>
                  <a href="#" > <i class="fas fa-edit" @click="updateModal(resourse)"> </i>
                  </a>
                    /
                  <a href="#" @click="deleteUser(resourse.id)"> <i class="fas fa-trash red"> </i>
                  </a>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input v-model="form.name" type="text" name="name"  placeholder="اسم المكون "
        ...
        <div class="form-group">
        <input v-model="form.packaging_type " type="text" name="packaging_type "  placeholder="نوع  الحزمة "
        ...
        <div class="form-group">
        <input v-model="form.quantity" type="number" name="quantity "  placeholder="الكمية "
...
<script>
    export default {
        ...
        methods:{
            ...
            updateModal(resourseDate){
                this.editeMode=true
                $('#addNewUserModal').modal('show')
                this.form.reset()
                this.form.fill(resourseDate)
            },
            ...
            updateCategory(id){
                this.$Progress.start()
              this.form.put('api/components/'+this.form.id)
                .then(()=>{
            ...
            deleteUser(id){
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        this.form.delete('api/components/'+id).then(()=>{
                            Swal.fire(
                                'Deleted!',
            ...
            getResults(page=1){
              axios.get('api/components?page='+page)
              .then(response=>{
                  this.resourses=response.data
              })
            },
          createCategory(){
              this.$Progress.start();
              this.form.post('api/components')
              .then(()=>{
